...
# Дата в самом начале строки-абзаца: ДД.ММ.ГГГГ (с «г.» или без)
_BARE_DATE_RE = re.compile(r'^\s*\d{2}\.\d{2}\.\d{4}\s*г?\.?')


def split_events(text):
    """Разбивает описание происшествий на отдельные эпизоды.

    В исходном документе каждый реальный эпизод — отдельный абзац (строка через
    перенос), начинающийся с даты (обычно с временем: ДД.ММ.ГГГГг. ... в ЧЧ:ММ).

    ВАЖНО: внутри одного эпизода может встречаться хронология с другими датами
    (напр. «26.07.2026г. в ходе конфликта ушёл из дома...»). Раньше скрипт резал
    по ЛЮБОЙ дате внутри текста и одно событие рассыпалось на несколько пунктов.
    Теперь разбиваем строго по границам абзацев (переносам строк): новый эпизод —
    только строка-абзац, начинающаяся с даты. Строки без даты в начале
    приклеиваются к текущему эпизоду.
    """
    if not text:
        return []

    raw_lines = [clean(l) for l in re.split(r'[\r\n]+', text)]
    raw_lines = [l for l in raw_lines if l]

    events = []
    for line in raw_lines:
        if _BARE_DATE_RE.match(line) or not events:
            events.append(line)
        else:
            events[-1] = clean(events[-1] + " " + line)

    return events if events else [clean(text)]
...
def parse(path):
    ...
    NEG_RE = re.compile(
        ...
        # «информация/факт/сообщение не подтвердилось/не подтвердился»
        r"не\s+подтвердил\w*|не\s+подтвержда\w*|не\s+нашл\w*\s+подтвержд|"
        r"подтверждения\s+не\s+наш|не\s+установлен\w*|"
        # ложный вызов / ложное сообщение
        r"ложн\w*\s+(?:вызов|сообщ|информ)|не\s+подтверд|информация\s+не\s+подтверд)",
        re.IGNORECASE,
    )
    ...
    def _is_fatal(text):
        if not text:
            return False
        # Опровержение в любой части текста отменяет летальность
        # (напр. «трупный запах... информация не подтвердилась»).
